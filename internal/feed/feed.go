...
func (r *ReleaseChecker) checkFeed(sem chan struct{}, projectName string, repo config.Repository) {
	...
	for retries := 0; retries < maxRetries; retries++ {
		r.logger.Info().Str("name", repo.Name).Msg("trying to fetch the feed")

		filepath := fmt.Sprintf("testdata/%s_releases.atom", projectName)
		...
		if err != nil {
			r.logger.Warn().Str("error", err.Error()).Msg("an error occurred while parsing feed, retrying...")
			continue
		}

		_ = file.Close()

		//feed, err := parser.ParseURL(fmt.Sprintf("%s/s3-manager_releases.atom", repo.Url))
		//if err != nil {
		//	logger.Warn().
		//		Str("error", err.Error()).
		//		Str("url", repo.Url).
		...
		fetchedReleases := r.getReleasesFromFeed(feed.Items)

		var diff []types.Release
		if aws.IsObjectExists(r.client, r.bucketName, fmt.Sprintf("%s_releases.json", projectName)) {
			previousReleases, err := aws.GetReleases(r.client, r.bucketName, fmt.Sprintf("%s_releases.json", projectName))
			if err != nil {
		...
		fmt.Println(diff)
		//logger.Info().Msg("putting diffs into bucket")
		//if err := aws.PutReleases(client, bucketName, fmt.Sprintf("%s_releases.json", projectName), diffReleases); err != nil {
		//	logger.Warn().Msg("an error occured while putting releases into bucket")
		//	continue
		//}

		// TODO: skip put operation if no diff found
		// TODO: notify if any diff found

		break
	}
}

func (r *ReleaseChecker) getReleasesFromFeed(items []*gofeed.Item) []types.Release {
...
func (r *ReleaseChecker) extractProjectName(repoUrl string) (string, error) {
	...
	// The project name is usually the second part of the path
	// Example: For "https://github.com/owner/projectname", it will return "projectname"
	return parts[2], nil
}

func (r *ReleaseChecker) contains(releases []types.Release, release types.Release) bool {
	for _, item := range releases {
		if release.Name == item.Name &&
			...
			release.IsNotified == item.IsNotified {
			return true
		}
	}
	return false
